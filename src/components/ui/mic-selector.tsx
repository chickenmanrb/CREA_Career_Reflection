...
export function MicSelector({
  ...
  useEffect(() => {
    if (value !== undefined && value !== selectedDevice) {
  ...
  // Reserve vertical space beneath the trigger while the dropdown is open so
  // it doesn't visually collide with content below (e.g., transcript cards).
  const estimatedMenuHeight = Math.min(devices.length * 40 + 120, 320)
  const reservedSpace = isDropdownOpen ? estimatedMenuHeight : 0

  return (
    <div className="relative" style={{ paddingBottom: reservedSpace }}>
