...
const STORAGE_KEYS = {
  name: "crea_acquisitions_candidate_name",
  email: "crea_acquisitions_candidate_email",
} as const;
...
export default function AcquisitionsInterviewPage() {
  const [messages, setMessages] = useState<TranscriptMessage[]>([]);
  const [currentStepId, setCurrentStepId] = useState<string>(acquisitionsFlowConfig.steps[0]?.id ?? "");
  const [persistState, setPersistState] = useState<PersistState>({ status: "idle" });

  const agentConfigured = acquisitionsFlowConfig.steps.some((s) => s.type === "agent" && s.agentId && s.agentId.length > 0);
  ...
  const currentStep = useMemo(
    () => acquisitionsFlowConfig.steps.find((s) => s.id === currentStepId) ?? acquisitionsFlowConfig.steps[0],
    [currentStepId]
  );
  ...
  const downloadTranscript = useCallback(() => {
    ...
    messages.forEach((msg) => {
      if (msg.stepId && msg.stepId !== lastStepId) {
        const stepMeta = acquisitionsFlowConfig.steps.find((s) => s.id === msg.stepId);
        if (stepMeta) {
    ...
    anchor.href = url;
    anchor.download = `acquisitions-reflection-transcript-${new Date().toISOString().replace(/[:.]/g, "-")}.txt`;
    anchor.click();
    URL.revokeObjectURL(url);
  }, [messages]);

  const promptSteps = useMemo(() => acquisitionsFlowConfig.steps.filter((s) => s.type === "agent"), []);
  ...
  const stepsForNav = useMemo(() => acquisitionsFlowConfig.steps, []);
  ...
  const persistTranscript = useCallback(async () => {
    ...
    try {
      const response = await fetch("/api/acquisitions/session", {
        method: "POST",
