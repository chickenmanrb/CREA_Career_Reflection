...
function allowedAgentIds() {
  const moduleConfig = getReflectionModule("development");
...
export async function GET(req: Request) {
  ...
  if (!allowed.has(agentId)) {
    return NextResponse.json({ error: "Agent not allowed for development" }, { status: 403 });
  }
  ...
  try {
    ...
    console.error("development signed-url error", error);
