...
function allowedAgentIds() {
  const moduleConfig = getReflectionModule("leasing-brokerage");
...
export async function GET(req: Request) {
  ...
  if (!allowed.has(agentId)) {
    return NextResponse.json({ error: "Agent not allowed for leasing brokerage" }, { status: 403 });
  }
  ...
  try {
    ...
    console.error("leasing-brokerage signed-url error", error);
