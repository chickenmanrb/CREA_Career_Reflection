...
function allowedAgentIds() {
  const moduleConfig = getReflectionModule("consulting");
...
export async function GET(req: Request) {
  ...
  if (!allowed.has(agentId)) {
    return NextResponse.json({ error: "Agent not allowed for consulting" }, { status: 403 });
  }
  ...
  try {
    ...
    console.error("consulting signed-url error", error);
