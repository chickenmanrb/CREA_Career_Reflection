...
function Scene({
  ...
  const random = useMemo(() => splitmix32(seed ?? 123456789), [seed])
  ...
  useFrame((_, delta: number) => {
    ...
    const nextTime = u.uTime.value + delta * 0.5
    const nextOpacity = Math.min(1, u.uOpacity.value + delta * 2)
  ...
  const uniforms = useMemo(() => {
    const perlin = perlinNoiseTexture.clone()
